...
class RADNeRFDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, prefix, data_dir=None, training=True):
        ...
        self.data_dir = (
            os.path.join(hparams["binary_data_dir"], hparams["video_id"])
            if data_dir is None
            else data_dir
        )
        ...
        self.cx = ds_dict["cx"]
        self.cy = ds_dict["cy"]
        # follow AD-NeRF, we dont use near-far in ds_dict
        ...
        if self.cond_type == "deepspeech":
            ...
            id, exp = (
                convert_to_tensor(ds_dict["id"]),
                convert_to_tensor(ds_dict["exp"]),
            )
            ...
            euler, trans = (
                convert_to_tensor(ds_dict["euler"]),
                convert_to_tensor(ds_dict["trans"]),
            )
