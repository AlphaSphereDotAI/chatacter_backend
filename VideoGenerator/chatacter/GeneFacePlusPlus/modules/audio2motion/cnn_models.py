    ...
    def __init__(
        ...
        self.blocks = [
            nn.Sequential(
                ...
                nn.GELU(),
                nn.Conv1d(
                    c_multiple * channels, channels, 1, dilation=dilation, bias=bias
                ),
            )
            for _ in range(n)
        ]
...
class ConvBlocks(nn.Module):
    ...
    def __init__(
        ...
        self.post_net1 = nn.Conv1d(
            channels, out_dims, kernel_size=3, padding=1, bias=bias
        )
        if init_weights:
...
class SeqLevelConvolutionalModel(nn.Module):
    ...
    def __init__(
        ...
        self.style_encoder = nn.Sequential(
            *[nn.Linear(135, 256), nn.GELU(), nn.Linear(256, 256)]
        )
    ...
    def forward(self, batch, ret, log_dict=None):
        ...
        if self.audio_feat_type == "ppg":
            ...
            audio_feat = self.audio_encoder(audio.transpose(1, 2)).transpose(
                1, 2
            ) * x_mask.unsqueeze(
                2
            )  # [B,T,C=29] => [B,T,C=48]
            energy_feat = self.energy_encoder(energy.transpose(1, 2)).transpose(
                1, 2
            ) * x_mask.unsqueeze(
                2
            )  # [B,T,C=1] => [B,T,C=16]
            ...
            feat = self.mel_encoder(mel.transpose(1, 2)).transpose(
                1, 2
            ) * x_mask.unsqueeze(
                2
            )  # [B,T,C=64]
        ...
        out = self.out_layer(feat.transpose(1, 2)).transpose(1, 2) * x_mask.unsqueeze(
            2
        )  # [B,T//2,C=256] => [B,T//2,C=64]
...
class ResBlocksBackbone(nn.Module):
    ...
    def __init__(self, in_dim=64, out_dim=512, p_dropout=0.5, norm_type="bn"):
        ...
        self.downsampler = LambdaLayer(
            lambda x: F.interpolate(x, scale_factor=0.5, mode="linear")
        )
        self.upsampler = LambdaLayer(
            lambda x: F.interpolate(x, scale_factor=4, mode="linear")
        )

        self.dropout = nn.Dropout(p=p_dropout)

    def forward(self, x, sty, x_mask=1.0):
        """
        x: [B, T, C]
        sty: [B, C=256]
        x_mask: [B, T]
