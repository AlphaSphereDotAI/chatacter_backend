...
class LFQ(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.project_in = (
            nn.Linear(dim, codebook_dims) if dim != codebook_dims else nn.Identity()
        )
        self.project_out = (
            nn.Linear(codebook_dims, dim) if dim != codebook_dims else nn.Identity()
        )
    ...
    def forward(self, x, mask=None, inv_temperature=1.0, return_loss_breakdown=False):
        """
        einstein notation
        b - batch
