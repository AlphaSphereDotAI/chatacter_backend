...
from .espnet_transformer_attn import (
...
class ConformerEncoder(ConformerLayers):
    ...
    def __init__(
        self, hidden_size, dict_size=0, in_size=0, strides=[2, 2], num_layers=None
    ):
    ...
    def forward(self, seq_out, mels_timbre, other_embeds=0):
        ...
        if self.dict_size != 0:
            x_mask = torch.unsqueeze(
                sequence_mask(
                    x_lengths + mels_timbre.size(1), x.size(1) + mels_timbre.size(1)
                ),
                2,
            ).to(x.dtype)
        else:
            x_mask = torch.cat(
                (
                    torch.ones(x.size(0), mels_timbre.size(1), 1).to(x.device),
                    (x.abs().sum(2) > 0).float()[:, :, None],
                ),
                dim=1,
            )
