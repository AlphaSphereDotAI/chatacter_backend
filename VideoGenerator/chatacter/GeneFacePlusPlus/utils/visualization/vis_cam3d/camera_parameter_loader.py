...
class CameraParameterLoader:
    ...
    def get_intrinsic(self, path):
        with open(os.path.join(path, "_camera_settings.json"), "r") as f:
            ...
            s = param_intrinsic["s"]
            mat_intrinsic = np.array([[fx, s, cx], [0, fy, cy], [0, 0, 1]])
    ...
    def get_extrinsic(self, path):
        with open(path, "r") as f:
            ...
            mat_translation = np.array(
                [[param_translation[0]], [param_translation[1]], [param_translation[2]]]
            )
            mat_extrinsic = np.concatenate(
                [
                    np.concatenate([mat_rotation, mat_translation], axis=1),
                    np.array([[0, 0, 0, 1]]),
                ],
                axis=0,
            )
