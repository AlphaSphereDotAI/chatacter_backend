...
index_mouth_from_lm478 = (
    index_innerlip_from_lm478 + index_outerlip_from_lm478 + index_withinmouth_from_lm478
)
...
class MediapipeLandmarker:
    ...
    def __init__(self):
        ...
        self.video_mode_options = vision.FaceLandmarkerOptions(
            base_options=base_options,
    ...
    def extract_lm478_from_img(self, img):
        img_landmarker = vision.FaceLandmarker.create_from_options(
            self.image_mode_options
        )
        ...
        H, W, _ = img.shape
        img_lm478 = np.array(img_face_landmarks)[:, :2] * np.array([W, H]).reshape(
            [1, 2]
        )  # [478, 2]
        return img_lm478
    ...
    def extract_lm478_from_video_name(self, video_name, fps=25, anti_smooth_factor=2):
        ...
        img_lm478, vid_lm478 = self.extract_lm478_from_frames(
            frames, fps, anti_smooth_factor
        )
    ...
    def extract_lm478_from_frames(self, frames, fps=25, anti_smooth_factor=20):
        ...
        vid_landmarker = vision.FaceLandmarker.create_from_options(
            self.video_mode_options
        )

        for i in range(len(frames)):
            frame = mp.Image(
                image_format=mp.ImageFormat.SRGB, data=frames[i].astype(np.uint8)
            )
            img_face_landmarker_result = img_landmarker.detect(image=frame)
            vid_face_landmarker_result = vid_landmarker.detect_for_video(
                image=frame, timestamp_ms=int((1000 / fps) * anti_smooth_factor * i)
            )
            try:
                img_ldm_i = img_face_landmarker_result.face_landmarks[0]
                ...
                print(
                    f"Warning: failed detect ldm in idx={i}, use previous frame results."
                )
            img_face_landmarks = np.array([[l.x, l.y, l.z] for l in img_ldm_i])
        ...
        img_lm478 = np.array(img_lm478)[..., :2] * np.array([W, H]).reshape(
            [1, 1, 2]
        )  # [T, 478, 2]
        vid_lm478 = np.array(vid_lm478)[..., :2] * np.array([W, H]).reshape(
            [1, 1, 2]
        )  # [T, 478, 2]
        return img_lm478, vid_lm478
